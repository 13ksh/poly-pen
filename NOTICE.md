# Third-party notices

## Nanum Pen Script fork

**Poly Pen is a fork of Nanum Pen Script.**
It is a Modified Version under the SIL Open Font License 1.1.
The family name is Poly Pen. It does not use the reserved names
Nanum or NanumPen.

Coverage matches the source font: Korean Hangul (11,172 syllables)
plus the rest of the Nanum Pen Script character map.
Styles: Regular 400 and Bold 700.

## Nanum Pen Script (SIL Open Font License 1.1)

Unmodified source file:

- `fonts/nanum/NanumPenScript-Regular.ttf`

Copyright (c) 2010, NHN Corporation.
Font designed by Sandoll Communications Inc.
Reserved Font Names: Nanum, Naver Nanum, NanumGothic, Naver NanumGothic,
NanumMyeongjo, Naver NanumMyeongjo, NanumBrush, Naver NanumBrush,
NanumPen, Naver NanumPen.

See `fonts/nanum/OFL.txt` and the project [OFL.txt](OFL.txt).
